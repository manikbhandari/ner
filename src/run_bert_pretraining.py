...
import argparse, logging, os, random, time, json, utils
from io import open
...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s', datefmt = '%m/%d/%Y %H:%M:%S', level = logging.INFO,
                    filename='logs/run_bert_pre_training{}.log'.format(time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())), filemode='a')
...
def main():
    ...
    if args.do_train:
        ...
        output_model_file = os.path.join(args.output_dir, "pretraining_{}.bin".format(time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())))

        if args.do_train: torch.save(model_to_save.state_dict(), output_model_file)
