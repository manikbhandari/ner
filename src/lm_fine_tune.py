...
logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -   %(message)s', datefmt = '%m/%d/%Y %H:%M:%S', level = logging.INFO,
                    filename='logs/lm_fine_tune_{}.log'.format(time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())), filemode='a')
...
def main():
    ...
    elif n_gpu > 1: # true most of the time
    ...
    if args.fp16:
        ...
        if args.loss_scale == 0: optimizer = FP16_Optimizer(optimizer, dynamic_loss_scale=True)
        else:                    optimizer = FP16_Optimizer(optimizer, static_loss_scale=args.loss_scale)
    ...
    if args.do_train:
        ...
        else:                     train_sampler = DistributedSampler(train_dataset)
