...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, pos_seq, bsz=None):
        ...
        if bsz is not None: return pos_emb[:,None,:].expand(-1, bsz, -1)
        else:               return pos_emb[:,None,:]
...
class PositionwiseFF(nn.Module):
    ...
    def forward(self, inp):
        if self.pre_lnorm:
            core_out = self.CoreNet(self.layer_norm(inp)) # layer normalization + positionwise feed-forward
            output   = core_out + inp                     # residual connection

        else:
            core_out = self.CoreNet(inp)                  # positionwise feed-forward
            output   = self.layer_norm(inp + core_out)    # residual connection + layer normalization
...
class MultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0, pre_lnorm=False):
        ...
        self.q_net  = nn.Linear(d_model, n_head * d_head, bias=False) # query
        self.kv_net = nn.Linear(d_model, 2 * n_head * d_head, bias=False) # key and values

        self.drop    = nn.Dropout(dropout)
        self.dropatt = nn.Dropout(dropatt)
        self.o_net   = nn.Linear(n_head * d_head, d_model, bias=False) # output

        self.layer_norm = nn.LayerNorm(d_model)

        self.scale = 1 / (d_head ** 0.5) # for scaled attnetion
...
class DecoderLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, **kwargs): #kwargs has dropatt and pre_lnorm
        super(DecoderLayer, self).__init__()

        self.dec_attn = MultiHeadAttn(n_head, d_model, d_head, dropout, **kwargs)
        self.pos_ff   = PositionwiseFF(d_model, d_inner, dropout, pre_lnorm=kwargs.get('pre_lnorm')) # FC -> ReLU -> FC  with layer norm and res
    ...
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner, dropout, dropatt, 
        ...
        if attn_type == 0:              # the default attention. this will have relative positional encoding.
            for i in range(n_layer): 
                self.layers.append(RelPartialLearnableDecoderLayer(n_head, d_model, d_head, d_inner, dropout,
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        if self.attn_type == 0: # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                if mems_i is not None and i == 0: mems_i += pos_emb[:mlen]
