import argparse, time, math, os, sys, itertools, ipdb as pdb
...
parser.add_argument('--init',                       default='normal',                                               help='parameter initializer to use.')
parser.add_argument('--emb_init',                   default='normal',                                               help='parameter initializer to use.')
...
parser.add_argument('--optim',                      default='adam',   choices=['adam', 'sgd', 'adagrad'],           help='optimizer to use.')
...
parser.add_argument('--scheduler',                  default='cosine', choices=['cosine', 'inv_sqrt', 'dev_perf', 'constant'], help='lr scheduler to use.')
...
parser.add_argument('--eval_tgt_len',   type=int,   default=50,                                                     help='number of tokens to predict for evaluation')
...
parser.add_argument('--work_dir',                   default='LM-TFM',                                               help='experiment directory.')
...
#By default embedding dimension is same as model dimension
if args.d_embed < 0: args.d_embed = args.d_model

assert args.ext_len >= 0, 'extended context length must be non-negative'
#Each batch will be divided into chunks of size batch_chunk to save memory
assert args.batch_size % args.batch_chunk == 0

#Add dataset and unique time to working directory
args.work_dir = '{}-{}'.format(args.work_dir, args.dataset)
args.work_dir = os.path.join(args.work_dir, time.strftime('%Y%m%d-%H%M%S'))
#Logging function with log path as work_dir, and scripts copied
...
ntokens      = len(corpus.vocab) # len(corpus.vocab.idx2sym)
args.n_token = ntokens           # Add n_token to args.

eval_batch_size = 10
# LMOrderedIterator for wt103. On interating, it will return input, target, seq_len
tr_iter = corpus.get_iterator('train', args.batch_size, args.tgt_len,      device=device, ext_len=args.ext_len)
va_iter = corpus.get_iterator('valid', eval_batch_size, args.eval_tgt_len, device=device, ext_len=args.ext_len)
te_iter = corpus.get_iterator('test',  eval_batch_size, args.eval_tgt_len, device=device, ext_len=args.ext_len)
...
if args.adaptive:
    ...
    if args.dataset == 'wt103':
        cutoffs   = [20000, 40000, 200000] # for adaptive softmax.    TODO: understnd this.
        tie_projs += [True] * len(cutoffs) # when to tie projections. TODO: understnd this.
...
def init_weight(weight):
    """
    Fills the weight tensor with initialized values.
    """
    if args.init == 'uniform':  nn.init.uniform_(weight, -args.init_range, args.init_range)
    elif args.init == 'normal': nn.init.normal_(weight, 0.0, args.init_std)
...
def weights_init(m):
    """
    Initialize the weights of model m
    """
    classname = m.__class__.__name__
    if classname.find('Linear') != -1:
        if hasattr(m, 'weight') and m.weight is not None: init_weight(m.weight)
        if hasattr(m, 'bias') and m.bias is not None:      init_bias(m.bias)
        ...
        if hasattr(m, 'weight'): init_weight(m.weight)

    elif classname.find('ProjectedAdaptiveLogSoftmax') != -1:
        if hasattr(m, 'cluster_weight') and m.cluster_weight is not None: init_weight(m.cluster_weight)
        if hasattr(m, 'cluster_bias')   and m.cluster_bias   is not None: init_bias(m.cluster_bias)

        if hasattr(m, 'out_projs'):
            for i in range(len(m.out_projs)):
                if m.out_projs[i] is not None: nn.init.normal_(m.out_projs[i], 0.0, args.proj_init_std)

    elif classname.find('LayerNorm') != -1:
        if hasattr(m, 'weight'): nn.init.normal_(m.weight, 1.0, args.init_std)
        if hasattr(m, 'bias') and m.bias is not None: init_bias(m.bias)

    elif classname.find('TransformerLM') != -1:
        if hasattr(m, 'r_emb'):    init_weight(m.r_emb)
        if hasattr(m, 'r_w_bias'): init_weight(m.r_w_bias)
        if hasattr(m, 'r_r_bias'): init_weight(m.r_r_bias)
        if hasattr(m, 'r_bias'):   init_bias(m.r_bias)

def update_dropout(m):
    classname = m.__class__.__name__
    if classname.find('Dropout') != -1:
        if hasattr(m, 'p'): m.p = args.dropout

def update_dropatt(m):
    # TODO: is this dropout for attention?
    if hasattr(m, 'dropatt'): m.dropatt.p = args.dropatt
...
if args.multi_gpu:
    ...
    if args.gpu0_bsz >= 0: para_model = BalancedDataParallel(args.gpu0_bsz // args.batch_chunk, model, dim=1).to(device)
    else:                  para_model = nn.DataParallel(model, dim=1).to(device)

else: para_model = model.to(device)
...
if args.optim.lower() == 'sgd':
    if args.sample_softmax > 0:
        ...
        for param in model.parameters():
            if param.size() == model.word_emb.weight.size(): sparse_params.append(param)
            else:                                            dense_params.append(param)

        optimizer_sparse = optim.SGD(sparse_params, lr=args.lr * 2)
        optimizer        = optim.SGD(dense_params, lr=args.lr, momentum=args.mom)

    else: optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=args.mom)

elif args.optim.lower() == 'adam':
    if args.sample_softmax > 0:
        dense_params, sparse_params = [], []
        for param in model.parameters():
            if param.size() == model.word_emb.weight.size(): sparse_params.append(param) # treat word embeddings as sparse. TODO: Why?
            else:                                            dense_params.append(param)

        optimizer_sparse = optim.SparseAdam(sparse_params, lr=args.lr)
        optimizer        = optim.Adam(dense_params, lr=args.lr)

    else: optimizer = optim.Adam(model.parameters(), lr=args.lr)
